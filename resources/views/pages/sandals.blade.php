<!DOCTYPE html>
<head>
    <link rel="stylesheet" href="css/sandals.css">
</head>

</body>
</html>


<!-- For more settings use the AutoHTML plugin tab ... --> 
<div class="sandals" >
    <div class="rectangle-2507" >
    </div>
    <div class="rectangle-2496" >
    </div>
    <img class="dhruv-i-1-yy-s-kk-8-kne-unsplash-1" src="gambar/sandal top.png" />
    <div class="sandals2" >
Sandals </div>
    <div class="s" >
S </div>
    <div class="avaliablitiy" >
Avaliablitiy </div>
    <div class="price" >
Price </div>
    <div class="rectangle-2504" >
    </div>
    <div class="in-stock-17" >
In stock (17) </div>
    <div class="rectangle-2505" >
    </div>
    <div class="out-of-stock-20" >
out of stock (20) </div>
    <div class="rp-0-00-rp-815-00-00" >
Rp.0,00     Rp.815.00,00  </div>
    <svg class="line-6" width="140" viewBox="0 0 140 0" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 0H140" stroke="black"/>
    </svg>
    <div class="vuesax-bold-arrow-left" >
          <div class="vuesax-bold-arrow-left2" >
                <svg class="arrow-left" width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M21.9993 3.66669C11.8793 3.66669 3.66602 11.88 3.66602 22C3.66602 32.12 11.8793 40.3334 21.9993 40.3334C32.1194 40.3334 40.3327 32.12 40.3327 22C40.3327 11.88 32.1194 3.66669 21.9993 3.66669ZM28.416 23.375H18.901L22.0543 26.5284C22.586 27.06 22.586 27.94 22.0543 28.4717C21.7793 28.7467 21.431 28.875 21.0827 28.875C20.7344 28.875 20.386 28.7467 20.111 28.4717L14.611 22.9717C14.0793 22.44 14.0793 21.56 14.611 21.0284L20.111 15.5284C20.6427 14.9967 21.5227 14.9967 22.0543 15.5284C22.586 16.06 22.586 16.94 22.0543 17.4717L18.901 20.625H28.416C29.1677 20.625 29.791 21.2484 29.791 22C29.791 22.7517 29.1677 23.375 28.416 23.375Z" fill="white"/>
                </svg>
          </div>
    </div>
    <div class="rectangle-11" >
    </div>
    <div class="vuesax-linear-arrow-right" >
          <div class="vuesax-linear-arrow-right2" >
                <svg class="arrow-right" width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M15.7188 20.75L8.92708 13.9583C8.125 13.1562 8.125 11.8438 8.92708 11.0417L15.7188 4.25" stroke="#FEFFFF" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
          </div>
    </div>
    <div class="rectangle-12" >
    </div>
    <div class="vuesax-linear-arrow-left" >
          <div class="vuesax-linear-arrow-left2" >
                <svg class="arrow-left2" width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M7.81358 17.4584L13.4733 11.7987C14.1417 11.1302 14.1417 10.0365 13.4733 9.3681L7.81358 3.70837" stroke="#FEFFFF" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
          </div>
    </div>
    <div class="rectangle-2509" >
    </div>
    <img class="image-43" src="gambar/image 43.png" />
    <div class="rectangle-2515" >
    </div>
    <div class="sale" >
Sale </div>
    <div class="garuda-broslide-white-green" >
Garuda Broslide White Green </div>
    <div class="group-33528" >
          <div class="rp-160-000-00-rp-90-000-00" >
Rp 160.000,00  Rp 90.000,00 </div>
          <svg class="vector-163" width="97" viewBox="0 0 97 0" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0 0H97" stroke="black"/>
          </svg>
    </div>
    <div class="rectangle-2506" >
    </div>
    <img class="unnamed-2-removebg-preview-2" src="gambar/sandal garuda.png" />
    <div class="group-33526" >
          <div class="rectangle-25152" >
          </div>
          <div class="sale2" >
Sale </div>
    </div>
    <div class="garuda-broslide-strap-red-white" >
Garuda Broslide strap Red White </div>
    <div class="rp-160-000-00-rp-95-000-00" >
Rp 160.000,00  Rp 95.000,00 </div>
    <svg class="vector-1632" width="97" viewBox="0 0 97 0" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 0H97" stroke="black"/>
    </svg>
    <div class="rectangle-2508" >
    </div>
    <div class="group-33530" >
          <div class="rectangle-25153" >
          </div>
          <div class="sale3" >
Sale </div>
    </div>
    <img class="unnamed-3-removebg-preview-2" src="gambar/sandal garuda 2.png" />
    <div class="garuda-broslide-white-red" >
Garuda Broslide White Red </div>
    <div class="rp-160-000-00-rp-145-000-00" >
Rp 160.000,00  Rp 145.000,00 </div>
    <svg class="line-7" width="96" viewBox="0 0 96 0" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 0H96" stroke="black"/>
    </svg>
    <div class="rectangle-2510" >
    </div>
    <div class="sale4" >
Sale </div>
    <img class="unnamed-4-removebg-preview-1" src="gambar/sandals 7.png" />
    <div class="garuda-broslide-strap-full-black" >
Garuda Broslide Strap Full Black </div>
    <div class="rp-160-000-00-rp-95-000-002" >
Rp 160.000,00  Rp 95.000,00 </div>
    <svg class="vector-1633" width="97" viewBox="0 0 97 0" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 0H97" stroke="black"/>
    </svg>
    <div class="rectangle-2512" >
    </div>
    <div class="frame-102" >
          <div class="sold-out" >
Sold Out </div>
    </div>
    <img class="unnamed-5-removebg-preview-1" src="gambar/sandals 8.png" />
    <div class="agni-dark-choco-bs" >
Agni Dark Choco BS </div>
    <div class="group-33546" >
          <div class="rp-175-000-00" >
Rp 175.000,00 </div>
    </div>
    <div class="rectangle-2514" >
    </div>
    <img class="unnamed-6-removebg-preview-1" src="gambar/sandals 10.png" />
    <div class="tama-cruise-dark-choco" >
Tama Cruise Dark Choco </div>
    <div class="group-33548" >
          <div class="rp-165-000-00" >
Rp 165.000,00 </div>
    </div>
    <div class="rectangle-2513" >
    </div>
    <div class="frame-1022" >
          <div class="sold-out" >
Sold Out </div>
    </div>
    <img class="unnamed-7-removebg-preview-1" src="gambar/sandals 9.png" />
    <div class="sapta-black-bs" >
Sapta Black BS </div>
    <div class="rp-475-000-00-rp-361-000-00" >
Rp 475.000,00  Rp 361.000,00 </div>
    <div class="rectangle-2511" >
    </div>
    <div class="rectangle-25154" >
    </div>
    <div class="sale5" >
Sale </div>
    <img class="unnamed-8-removebg-preview-1" src="gambar/sandals six.png" />
    <div class="masimo-slider-full-black" >
Masimo Slider Full Black </div>
    <div class="rp-175-000-00-rp-90-000-00" >
Rp 175.000,00  Rp 90.000,00 </div>
    <svg class="vector-165" width="94" viewBox="0 0 94 0" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 0H94" stroke="black"/>
    </svg>
    <div class="rectangle-25155" >
    </div>
    <img class="image-42" src="gambar/image 42.png" />
    <div class="group-33527" >
          <div class="rectangle-25156" >
          </div>
          <div class="sale6" >
Sale </div>
    </div>
    <div class="garuda-broslide-strap-green" >
Garuda Broslide Strap Green </div>
    <div class="rp-160-000-00-rp-95-000-003" >
Rp 160.000,00  Rp 95.000,00 </div>
    <svg class="vector-1634" width="104" viewBox="0 0 104 0" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 0H103.5" stroke="black"/>
    </svg>
    <svg class="vector-166" width="97" viewBox="0 0 97 0" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 0H97" stroke="black"/>
    </svg>
</div>